...
const CreateTweet: React.FC = () => {
  ...
  const handleGenerate = useCallback(async (type: 'tweet' | 'thread') => {
    ...
    if (type === 'tweet') {
        const result = await generateTweet(prompt, sourceToUse, audienceToUse);
        setTweets([result]);
    } else {
        const results = await generateTweetThread(prompt, sourceToUse, audienceToUse);
                ...
                <div className="mb-4">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-grow bg-bg-primary border border-border-primary rounded-lg p-3 focus:ring-2 focus:ring-accent-primary focus:shadow-glow-blue focus:outline-none transition"
                        />
            ...
            {tweets.map((tweet, index) => (
              <div key={index} className="relative">
                <textarea
                  ...
                  className="w-full bg-bg-primary border border-border-primary rounded-lg p-3 pr-16 focus:ring-2 focus:ring-accent-primary focus:shadow-glow-blue focus:outline-none transition"
                  rows={4}
                />
                <span className={`absolute bottom-3 right-3 text-sm ${tweet.length > MAX_CHARS ? 'text-accent-secondary' : 'text-text-secondary'}`}>
