...
const GenerationStatus: React.FC<GenerationStatusProps> = ({ title, steps, currentStepIndex, error }) => {
  ...
  const renderErrorState = () => (
    <div className="bg-danger/10 border border-danger/50 rounded-lg p-6 text-center animate-fade-in">
        ...
        <h3 className="font-bold text-lg text-danger mb-2">Generation Failed</h3>
          ...
          return (
            <div key={index} className="flex items-center space-x-4 transition-opacity duration-300" style={{opacity: isCompleted ? 0.7 : 1}}>
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${isCurrent ? 'border-accent-primary shadow-glow-blue' : isCompleted ? 'border-success bg-success/20' : 'border-border-primary'}`}>
                  {isCompleted ? (
  ...
  return (
    <div className="bg-bg-primary border border-border-primary rounded-lg animate-fade-in min-h-[300px] flex flex-col justify-center">
        ...
        <style>{`
            ...
            @keyframes glow {
                0%, 100% { box-shadow: 0 0 2px #3B82F6, 0 0 5px #10B981; }
                50% { box-shadow: 0 0 8px #3B82F6, 0 0 20px #10B981; }
            }
