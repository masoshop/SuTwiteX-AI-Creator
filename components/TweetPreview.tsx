      ...
      <div className="flex-1">
        ...
        {isEditable && (
            <div className="mt-4 pt-4 border-t border-border-primary/50 flex items-center justify-end gap-2 flex-wrap">
                ...
                {/* Content Actions */}
                <button onClick={onCopy} className="action-button w-24" title="Copy Text" disabled={isGenerating || editableTweet.isRegenerating}>
                    {editableTweet.isCopied ? <><CheckIcon/> Copied!</> : <><CopyIcon /> Copy</>}
                </button>
                <button onClick={onRegenerate} className="action-button" title="Regenerate Tweet" disabled={!editableTweet.content || isGenerating || editableTweet.isRegenerating}>
                ...
                {!editableTweet.media ? (
                    <>
                    <button onClick={onUploadMedia} className="action-button" title="Upload Media" disabled={isGenerating || editableTweet.isRegenerating}><UploadIcon /></button>
                    <button onClick={() => onGenerateMedia('image')} disabled={!editableTweet.content || isGenerating || editableTweet.isRegenerating} className="action-button" title="Generate Image"><CameraIcon /></button>
                    <button onClick={() => onGenerateMedia('video')} disabled={!editableTweet.content || isGenerating || editableTweet.isRegenerating} className="action-button" title="Generate Video"><VideoIcon /></button>
                    </>
                ) : (
                    <>
                    <button onClick={onUploadMedia} className="action-button" title="Change Media" disabled={isGenerating || editableTweet.isRegenerating}><UploadIcon /></button>
                    <button onClick={onRemoveMedia} className="action-button !bg-danger/70 hover:!bg-danger/90" title="Remove Media"><TrashIcon /></button>
                    </>
                )}
                ...
                {/* Tweet Actions */}
                <button onClick={onDeleteTweet} className="action-button !bg-danger/70 hover:!bg-danger/90" title="Delete Tweet" disabled={isGenerating || editableTweet.isRegenerating}>
                    <TrashIcon />
                </button>
            </div>
        )}
